    ...
    */
    static void HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo, ChatHandler* handler)
    {
        ...
        if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
            sWorld->AddCharacterNameData(delInfo.lowGuid, delInfo.name, (*result)[2].GetUInt8(), (*result)[0].GetUInt8(), (*result)[1].GetUInt8(), (*result)[3].GetUInt8(), (*result)[4].GetUInt32());
    }
    ...
    static bool HandleCharacterRenameCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (newNameStr)
        {
            ...
            if (ObjectMgr::CheckPlayerName(newName, true) != ResponseCodes::CHAR_NAME_SUCCESS)
            {
                handler->SendSysMessage(LANG_BAD_VALUE);
                handler->SetSentErrorMessage(true);
                return false;
