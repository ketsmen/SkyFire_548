    ...
    for (std::vector<GameObject*>::iterator itr = _gameObjectsToMove.begin(); itr != _gameObjectsToMove.end(); ++itr)
    {
        ...
        else
        {
            ...
            if (!GameObjectRespawnRelocation(go, false))
            {
                // ... or unload (if respawn grid also not loaded)
#ifdef SKYFIRE_DEBUG
                SF_LOG_DEBUG("maps", "GameObject (GUID: %u Entry: %u) cannot be move to unloaded respawn grid.", go->GetGUIDLow(), go->GetEntry());
#endif
...
bool GridMap::loadLiquidData(FILE* in, uint32 offset, uint32 /*size*/)
{
    ...
    {
        m_liquidMap = new float[uint32(m_liquidWidth) * uint32(m_liquidHeight)];
        if (fread(m_liquidMap, sizeof(float), m_liquidWidth * m_liquidHeight, in) != (uint32(m_liquidWidth) * uint32(m_liquidHeight)))
            return false;
    }
    return true;
}
    ...
    if (m_liquidEntry)
    {
        if (LiquidTypeEntry const* liquidEntry = sLiquidTypeStore.LookupEntry(m_liquidEntry[idx]))
        {
            ...
            if (entry < 21)
            {
                ...
                {
                    uint32 overrideLiquid = area->m_LiquidType[liquidEntry->Type];
                    if (!overrideLiquid && area->m_ParentAreaID)
                    {
                        area = GetAreaEntryByAreaID(area->m_ParentAreaID);
                        if (area)
                            overrideLiquid = area->m_LiquidType[liquidEntry->Type];
                    }
...
inline bool IsOutdoorWMO(uint32 mogpFlags, int32 /*adtId*/, int32 /*rootId*/, int32 /*groupId*/, WMOAreaTableEntry const* wmoEntry, AreaTableEntry const* atEntry)
{
    ...
    if (wmoEntry && atEntry)
    {
        if (atEntry->m_flags & AREA_FLAG_OUTSIDE)
            return true;
        if (atEntry->m_flags & AREA_FLAG_INSIDE)
            return false;
    }
        ...
        if (liquid_level > ground_level && z > ground_level - 2)
        {
            ...
            if (data)
            {
                ...
                if (liquid_type && liquid_type < 21)
                {
                    ...
                    {
                        uint32 overrideLiquid = area->m_LiquidType[liquidFlagType];
                        if (!overrideLiquid && area->m_ParentAreaID)
                        {
                            area = GetAreaEntryByAreaID(area->m_ParentAreaID);
                            if (area)
                                overrideLiquid = area->m_LiquidType[liquidFlagType];
                        }
...
uint32 Map::GetZoneIdByAreaFlag(uint16 areaflag, uint32 map_id)
{
    ...
    if (entry)
        return (entry->m_ParentAreaID != 0) ? entry->m_ParentAreaID : entry->m_ID;
    else
        return 0;
}
...
void Map::GetZoneAndAreaIdByAreaFlag(uint32& zoneid, uint32& areaid, uint16 areaflag, uint32 map_id)
{
    ...
    areaid = entry ? entry->m_ID : 0;
    zoneid = entry ? ((entry->m_ParentAreaID != 0) ? entry->m_ParentAreaID : entry->m_ID) : 0;
}
    ...
    i_data(NULL), i_script_id(0)
{
    ...
    m_unloadTimer = std::max(sWorld->getIntConfig(WorldIntConfigs::CONFIG_INSTANCE_UNLOAD_DELAY), (uint32)MIN_UNLOAD_DELAY);
}
...
void InstanceMap::RemovePlayerFromMap(Player* player, bool remove)
{
    ...
    if (!m_unloadTimer && m_mapRefManager.getSize() == 1)
        m_unloadTimer = m_unloadWhenEmpty ? MIN_UNLOAD_DELAY : std::max(sWorld->getIntConfig(WorldIntConfigs::CONFIG_INSTANCE_UNLOAD_DELAY), (uint32)MIN_UNLOAD_DELAY);
